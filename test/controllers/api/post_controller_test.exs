defmodule Shelve.API.PostControllerTest do
  ...
  test "creates and renders resource when data is valid", %{conn: conn} do
    conn = post conn, api_post_path(conn, :create), post: @valid_attrs
  ...
  test "updates and renders chosen resource when data is valid", %{conn: conn} do
    post = Repo.insert! struct(Post, @valid_attrs)
    conn = patch conn, api_post_path(conn, :update, post), post: @valid_attrs
  ...
  test "does not update chosen resource and renders errors when data is invalid", %{conn: conn} do
    post = Repo.insert! struct(Post, @valid_attrs)
    conn = patch conn, api_post_path(conn, :update, post), post: @invalid_attrs
    assert json_response(conn, 422)["errors"] != %{}
  end
